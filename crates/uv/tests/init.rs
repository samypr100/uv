...
fn init_app_build_backend_meson() -> Result<()> {
    ...
    uv_snapshot!(context.filters(), context.init().current_dir(&child).arg("--app").arg("--package").arg("--build-backend").arg("meson"), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    insta::with_settings!({
        ...
        install_subdir('src/foo', install_dir: py.get_install_dir() / 'foo', strip_directory: true)
        "###
        );
    });
...
fn init_lib_build_backend_meson() -> Result<()> {
    ...
    uv_snapshot!(context.filters(), context.init().current_dir(&child).arg("--lib").arg("--build-backend").arg("meson"), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
